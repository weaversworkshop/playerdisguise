...
- **Server-side alias blocklist.** `AliasContentFilter` loads `<world>/playerdisguise/alias-blocklist.txt` (admin-editable, ships empty with a comment header). Case-insensitive substring match; admins add leet/punctuation variants as explicit lines. Hooked into the config-phase claim handler in `PdNetworkSetup#handleChoiceConfig` — a blocked alias disconnects the player before `AliasRegistry.claim()` runs (no PLAY entry, no broadcast trail). Server log records the rejection with the player's real name.
- **`/reportdisguise <alias> [reason]`.** Players submit reports against currently-disguised aliases; entries append to `<world>/playerdisguise/reports.log` (TSV: timestamp, reporter UUID+real name, target UUID+real name, alias, reason) and emit a server-log line. Silent — no chat broadcast, no `/reports` command. Resolves the target via `AliasRegistry.uuidOf`; rejects if no one currently holds the alias.

### Phase 4 — storage hygiene + two-step skin handshake
- **Two-step skin handshake.** Client sends `ClientDisguiseChoice(name, hash, model)` with **no bytes**. Server checks `SkinStore.has(hash)`; if hit, commits immediately. If miss, server replies `ServerNeedsSkinUpload(hash)`; client replies `ClientSkinUpload(hash, bytes)`; server validates and commits. On validation/hash-mismatch failure, server replies `SkinUploadFailed(reason)` and holds the connection. Client shows a `ConfirmScreen` with `GUI_PROCEED`/`GUI_CANCEL` buttons; Cancel disconnects (alias never committed — nothing to roll back); Proceed sends `ProceedWithoutSkin` and server commits alias-only.
- **Anti-spam guards.** Per-listener `PENDING` map on the server holds the expected hash. Unsolicited `ClientSkinUpload` → disconnect. Hash mismatch → fail path. Single upload attempt per claim handshake. `SkinValidator` (PNG/dims/≤64KB/hash match) still gates `store`.
- **Persistent skin refs in `aliases.json`.** `ActiveEntry` and `SavedCooldown` extended with `(hash, model)` (nullable). Cooldown carries the just-released skin so a quick rejoin within the cooldown window resumes without re-uploading. `AliasRegistry.liveSkinHashes()` is the union of active + cooldown hashes — the GC live set.
- **Server-side GC + LRU** (`SkinStore`). `gcOrphans(liveHashes)` deletes blobs not in `AliasRegistry.liveSkinHashes()`. `enforceCap()` is a soft 5000-blob LRU (~320 MB worst case) that evicts oldest-mtime even if referenced — robust because the upload-on-demand flow handles cache misses gracefully. `load()` and `store()` touch mtime. Both run on `ServerStoppingEvent`, every 10 min via `ServerPersistenceHook`, and `enforceCap` also runs after each successful upload.
